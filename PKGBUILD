...
package() {
    ...
    install -Dpm755 -D "${pkgname}" "$pkgdir/usr/bin/mopyqtiv"

    gzip -9 "${pkgname}.1"
    install -Dpm644 -D "${pkgname}.1.gz" "$pkgdir/usr/share/man/ru/man1/mopyqtiv.1.gz"

    install -Dpm644 -D "${pkgname}.desktop" "$pkgdir/usr/share/applications/mopyqtiv.desktop"
}
